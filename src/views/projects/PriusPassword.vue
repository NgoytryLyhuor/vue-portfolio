        ...
        <div class="max-w-md w-full">
            <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-8 sm:p-10 border border-gray-200 dark:border-gray-700">
                <div class="text-center mb-8">
                    ...
                    <p class="text-gray-600 dark:text-gray-400 text-sm sm:text-base">
                        Enter password to access Prius Care
                    </p>
                </div>
                ...
                <form @submit.prevent="handleSubmit" class="space-y-6">
                    <div>
                        ...
                        <div class="relative">
                            <input
                                ...
                                class="w-full px-4 py-3 pr-12 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all"
                    ...
                    <button
                        ...
                        class="w-full py-3 px-4 bg-green-600 hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-semibold rounded-lg transition-colors duration-200 flex items-center justify-center gap-2 shadow-lg"
                    >
                        ...
                        <span v-if="!loading">Access Prius Care</span>
                ...
                <div class="mt-6 text-center">
                    <router-link
                        to="/"
                        class="text-sm text-gray-600 dark:text-gray-400 hover:text-green-600 dark:hover:text-green-400 transition-colors inline-flex items-center gap-1"
                    >
            ...
            <div class="mt-6 text-center">
                <p class="text-xs text-gray-500 dark:text-gray-400">
                    🔒 Private page for personal Prius maintenance tracking
                </p>
            </div>
        </div>
    </div>
...
<script setup>
// Password Protection Page for Prius Care
...
const PRIUS_PASSWORD = 'lyhuor'
const AUTH_KEY = 'prius_care_auth'
...
const handleSubmit = async () => {
    ...
    try {
        if (password.value.trim() === PRIUS_PASSWORD) {
            sessionStorage.setItem(AUTH_KEY, 'true')
            router.push('/prius-care')
        } else {
            errorMessage.value = 'Incorrect password. Please try again.'
            password.value = ''
        }
    } catch (err) {
        logger.error('Prius password verification error:', err)
