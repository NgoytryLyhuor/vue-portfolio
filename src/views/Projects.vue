        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div class="mb-16">
                <div class="mb-8 flex flex-wrap justify-center gap-3">
                    <button @click="activeCategory = 'all'" :class="[ 'px-4 py-2 rounded-full text-sm font-medium transition-colors duration-300', activeCategory === 'all' ? 'bg-blue-600 text-white shadow-md' : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600 shadow-sm' ]">
                        All
                    </button>
                    <button v-for="category in categories" :key="category" @click="activeCategory = category" :class="[ 'px-4 py-2 rounded-full text-sm font-medium transition-colors duration-300', activeCategory === category ? 'bg-blue-600 text-white shadow-md' : 'bg-white dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-600 shadow-sm' ]">
                        {{ category }}
                    </button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                <article v-for="project in filteredProjects" :key="project.id"
                    ...
                    <div class="h-48 bg-gray-100 dark:bg-gray-700 relative overflow-hidden">
                        ...
                        <img :src="isExternalImage(project.image) ? project.image : require(`@/assets/${project.image}`)" :alt="project.title" class="w-full h-full object-cover" @error="handleImageError">
                        <div class="absolute bottom-4 left-4">
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-200 shadow-sm">
                                {{ project.category }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="p-6 flex-1 flex flex-col">
                        ...
                        <div class="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between gap-4">
                            <a :href="project.demo"
                            ...
                            <a v-if="project.showGithub !== false && project.github !== '#'" :href="project.github" class="flex-1 text-center px-4 py-2 bg-gray-800 hover:bg-gray-900 text-white font-medium rounded-lg transition-colors duration-300 text-sm dark:bg-gray-700 dark:hover:bg-gray-600">
                                GitHub
                            </a>
                        </div>
                    </div>
            ...
            <div v-if="filteredProjects.length === 0" class="text-center py-12">
                <div class="mx-auto max-w-md">
                    ...
                    <p class="text-gray-500 dark:text-gray-500">We couldn't find any projects matching this category. </p>
                </div>
            </div>
        </div>
    </div>
